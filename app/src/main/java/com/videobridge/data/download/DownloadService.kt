...
class DownloadService : Service() {
    ...
    private fun startObservingWithResume() {
        val downloader = (application as? JellyfinBridgeApplication)?.container?.downloader
        ...
        observerJob = serviceScope.launch {
            ...
            runCatching { downloader.resumePending() }
                .onFailure {
                    (application as? JellyfinBridgeApplication)?.container?.logger?.w("download", "resumePending failed", it)
                }
    ...
    private fun handleTimeout() {
        ...
        runCatching {
            (application as? JellyfinBridgeApplication)?.logger
    ...
    companion object {
        private const val ACTION_START = "com.videobridge.action.DOWNLOAD_START"
        private const val ACTION_STOP = "com.videobridge.action.DOWNLOAD_STOP"
